...
def validate_vehicle_text(value):
    ...
    if len(text) < 2 or len(text) > 40:
        print("Deve ter entre 2 e 40 caracteres.")
        return False

    if text.isdigit():
        print("Não pode ser apenas números.")
        return False

    if not re.fullmatch(r"[A-Za-zÀ-ÿ0-9 ]+", text):
        print("Use apenas letras e números (sem símbolos).")
        return False
